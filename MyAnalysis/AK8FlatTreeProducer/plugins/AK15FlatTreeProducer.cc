...
AK15FlatTreeProducer::AK15FlatTreeProducer(const edm::ParameterSet& iConfig) {
  recoJetsToken_ = consumes<JetCollection>(edm::InputTag("selectedPatJetsAK15PFCHS", "", "NANO"));
...
void AK15FlatTreeProducer::registerBranches(TTree* tree) {
...
void AK15FlatTreeProducer::beginJob() {
  outFile_train_ = new TFile("ak15_train.root", "RECREATE");
  outFile_val_ = new TFile("ak15_val.root", "RECREATE");
  outFile_test_ = new TFile("ak15_test.root", "RECREATE");

  outFile_train_->cd();
  tree_train_ = new TTree("Events", "Training events");
  registerBranches(tree_train_);

  outFile_val_->cd();
  tree_val_ = new TTree("Events", "Validation events");
  registerBranches(tree_val_);

  outFile_test_->cd();
  tree_test_ = new TTree("Events", "Test events");
...
void AK15FlatTreeProducer::analyze(const edm::Event& iEvent, const edm::EventSetup&) {
  ...
  if (recoJets.isValid()) {
    ...
    if (selectedJet) {
      ...
      if (nPart_ > 0) {
        if (eventCounter_ % 10 == 0) tree_test_->Fill();
        else if (eventCounter_ % 10 == 9) tree_val_->Fill();
        else tree_train_->Fill();
      }
    }
  }
}

void AK15FlatTreeProducer::endJob() {
  outFile_train_->cd();
  tree_train_->Write();
